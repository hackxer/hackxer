...
#    # #    #  ####  #    # #    # ###### #    #
  # Tool For Hacking
 # hackxer v1.0
# By ala abdellawi - instagram.com/ala_abdellawi
'''
Imports
'''
import sys
...
configFile = installDir + "/hackxer.cfg"
print(installDir)
config = ConfigParser.RawConfigParser()
config.read(configFile)

toolDir = installDir + config.get('hackxer', 'toolDir')
logDir = installDir + config.get('hackxer', 'logDir')
yes = config.get('hackxer', 'yes').split()
color_random=[color.HEADER,color.IMPORTANT,color.NOTICE,color.OKBLUE,color.OKGREEN,color.WARNING,color.RED,color.END,color.UNDERLINE,color.LOGGING]
random.shuffle(color_random)
hackxerlogo = color_random[0] + ''' 
/ \ / \ / \ / \ / \ / \ / \ 
( h | a | c | k | x | e | r )
 \_/ \_/ \_/ \_/ \_/ \_/ \_/ 
hackxerPrompt = "hackxer ~# "
...
'''
def agreement():
    while not config.getboolean("hackxer", "agreement"):
        clearScr()
        print(termsAndConditions)
        agree = raw_input("You must agree to our terms and conditions first (Y/n) ").lower()
        if agree in yes:
            config.set('hackxer', 'agreement', 'true')
            class hackxer:
    def __init__(self):
        clearScr()
        self.createFolders()
        print (fsocietylogo + color.RED + '''
       }--------------{+} Coded By ala abdellawi {+}--------------{
       }--------{+}  GitHub.com/hackxer/hackxer {+}--------{
    ''' + color.END + '''
       ...
       {99}-EXIT\n
     ''')
      choice = raw_input(hackxerPrompt)
        clearScr()
        if choice == "1":
    ...
    def __init__(self):
        ...
        choiceweb = raw_input(hackxerPrompt)
        clearScr()
        if choiceweb == "1":
            maine()
        elif choiceweb == "2":
...
class postExploitationMenu:
    ...
    def __init__(self):
        ...
        choice11 = raw_input(hackxerPrompt)
        clearScr()
        if choice11 == "1":
            sitechecker()
        elif choice11 == "2":
...
class informationGatheringMenu:
    ...
    def __init__(self):
        ...
        print("  {8}--Crips\n  ")
        print("  {99}-Back To Main Menu \n")
        choice2 = raw_input(hackxerPrompt)
        clearScr()
        if choice2 == "1":
            nmap()
        elif choice2 == "2":
...
class wirelessTestingMenu:
    ...
    def __init__(self):
        ...
        choice4 = raw_input(hackxerPrompt)
        clearScr()
        if choice4 == "1":
            reaver()
        elif choice4 == "2":
...
class exploitationToolsMenu:
    ...
    def __init__(self):
        ...
        choice5 = raw_input(hackxerPrompt)
        clearScr()
        if choice5 == "1":
            atscan()
        elif choice5 == "2":
